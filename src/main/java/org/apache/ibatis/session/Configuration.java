 ...
 * 　从设计上来说，我们可以说Configuration并不是一个thin类(也就是仅包含了属性以及getter/setter)，
 *    而是一个rich类，它对部分逻辑进行了封装便于用户直接使用,而不是让用户各自散落处理，比如addResultMap方法和getMappedStatement方法等等。
 */
 ...
 *  读取配置文件，包括基础配置XML文件和映射器的XML
 *  初始化基础配置，比如MyBatis的别名等，一些重要的的类对象，例如插件、映射器、ObjectFactory和TypeHandler对象
