...
public class XMLMapperBuilder extends BaseBuilder {
  ...
  public void parse() {
    ...
    if (!configuration.isResourceLoaded(resource)) {
      ...
      bindMapperForNamespace();//注册 Mapper
    }
  ...
  private void parsePendingStatements() {
    ...
    synchronized (incompleteStatements) { //加锁同步
      ...
      while (iter.hasNext()) {
        try {
          iter.next().parseStatementNode(); //II 重新解析 SQL 语句节点
  ...
  private void cacheElement(XNode context) {
    if (context != null) {
      //获取＜cache＞节点的type属性性，默认位是 PERPETUAL
      String type = context.getStringAttribute("type", "PERPETUAL");
      //查找 type 属性对应的Cache接口实现，TypeAliasRegistry 的实现前面介绍过了，不再赘述
      Class<? extends Cache> typeClass = typeAliasRegistry.resolveAlias(type);
      //获取＜cache>节点的 eviction 属性，默认位是 LRU
      String eviction = context.getStringAttribute("eviction", "LRU");
      //解析eviction属性指定的Cache装饰器类型
      Class<? extends Cache> evictionClass = typeAliasRegistry.resolveAlias(eviction);
      //获取＜cache＞节点的 flushinterval属性，默认值是 null
