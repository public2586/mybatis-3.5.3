...
public class XMLIncludeTransformer {
  ...
  public void applyIncludes(Node source) {
    ...
    applyIncludes(source, variablesContext, false); //处理include子节点
  }
   ...
   */
  //下面是处理＜include＞节点的 applyIncludes()方法重载：
  private void applyIncludes(Node source, final Properties variablesContext, boolean included) {
    if (source.getNodeName().equals("include")) {  //处理＜include＞子节点
      //查找 refid 属性指向 <sql> 节点,返回的是其深克隆的 Node 对象
      Node toInclude = findSqlFragment(getStringAttribute(source, "refid"), variablesContext);
      //解析 <include> 节点下的 <property> 节点，将得到的键值对添加到 variablesContext 中
      //并形成新的 Properties 对象返回，用于替换占位符
      Properties toIncludeContext = getVariablesContext(source, variablesContext);
      //递归处理＜include＞节点， sql节点中可能会使用 ＜include＞引用了其他 SQL 片段
      ...
      NodeList children = source.getChildNodes(); //遍历当前 SQL 语句的子节点
